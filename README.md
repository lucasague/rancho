...
- **Gestión de pendientes**: el historial guarda por orden la cantidad, método de pago y variantes pedidas. Con esa información se construyen vistas agrupadas por cliente y por producto. Cada línea puede marcarse como servida directamente desde el panel de pendientes o desde la tabla principal. Ahora cada producto del historial muestra controles por unidad: puedes tocar los iconos o los botones numerados para marcar entregas parciales (por ejemplo, servir solo 2 de 3 empanadas) o usar el botón de «fila completa» para alternar todo el renglón de una vez. El panel de pendientes se actualiza al instante con esas unidades entregadas.
...
2. **Registra pedidos** desde la pestaña principal. Usa “Enviar” para cobros inmediatos o “Pendiente” para acumular en una cuenta abierta. Marca cada producto (o cada unidad individual) como servido para mantener el panel de pendientes al día.
